...
export default function Navbar() {
  ...
  return (
    <motion.div
      ...
      className={`${isNavOpen ? 'shadow-md' : null}`}
    >
      <div
        className={`flex flex-row justify-between h-16 px-3 md:p-10 items-center w-screen ${
          isNavOpen ? null : 'shadow-md'
        }`}
      >
        <Link
          onClick={() => {
...
function MobileNavbar(isNavOpen, setIsNavOpen, pathName) {
  return (
    <motion.div
      ...
      } flex flex-col mx-3 mb-3 bg-[#fffffc] dark:bg-neutral-800 min-h-20 text-gray-500 dark:text-yellow-50 text-xl font-bold items-center`}
    >
      <Link
        onClick={() => {
          setIsNavOpen(false);
