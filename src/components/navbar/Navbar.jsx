...
export default function Navbar() {
  ...
  return (
    <div className={isNavOpen? 'shadow-md' : null}>
      ...
      {
        <motion.div initial={{opacity: 0, scale:0}} animate={isNavOpen? { opacity: 1, scale : 1 } : {opacity: 0, scale: 0}} transition={{ delay: 0.01, type: "spring"}} className={`md:hidden ${isNavOpen ? null : 'hidden'} flex flex-col mx-3 mb-2 shadow-md bg-gray-800 dark:bg-yellow-50 min-h-20 text-yellow-50 dark:text-gray-500 text-xl font-bold items-center`}>
            <Link onClick={navigationToggle} className={`md:hidden mb-2 mt-5 hover:text-yellow-200 dark:hover:text-gray-800 ${pathName === '/about' ? 'text-yellow-200 underline decoration-4 underline-offset-4 dark:text-neutral-800 hover:cursor-pointer' : null}`} href='/about'>About</Link>
            <Link onClick={navigationToggle} className={`md:hidden mb-2 hover:text-yellow-200 dark:hover:text-gray-800 ${pathName === '/projects' ? 'text-yellow-200 underline decoration-4 underline-offset-4 dark:text-neutral-800 hover:cursor-pointer' : null}`} href='/projects'>Projects</Link>
            <Link onClick={navigationToggle} className={`md:hidden mb-5 hover:text-yellow-200 dark:hover:text-gray-800 ${pathName === '/contact' ? 'text-yellow-200 underline decoration-4 underline-offset-4 dark:text-neutral-800 hover:cursor-pointer' : null}`} href='/contact'>Contact</Link>
        </motion.div>
      }
    </div>
